...
## 6. Bun 관련 명령어
- **Dev Server**: `bun run dev` (Hono, Watch Mode 구현됨)
...
## 8. 버전 관리 및 커밋/푸시 규칙
- **버전 업데이트**: 사용자가 명시적으로 릴리즈(Release) 또는 버전 업(Bump up)을 요청할 때만 `package.json`의 `version` 값을 상황에 맞게 1단계 올립니다. 평소 코드로직 수정 시에는 버전을 올리지 않습니다.
  - 기능 추가, 개선, 버그 수정 등 소소한 수정 사항은 주로 패치 버전(예: `1.0.1` ➡️ `1.0.2`)을 올립니다.
- **버전 표시**: 화면 우측 하단에 게임 버전이 항상 동적으로 표시되도록 보장해야 합니다. (`package.json`을 프론트엔드에서 Import 하여 UI에 렌더링)
