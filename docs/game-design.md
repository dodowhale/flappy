...
- **피버 게이지 및 피버 타임 (Fever Gauge & Fever Time)**:
  - 파이프 통과(Good: +5%, Perfect: +25%), 코인 획득(+4%), 보스 타격(+2%) 시 피버 게이지가 누적됩니다.
  - 게이지가 100%에 도달하면 5초간 자동으로 무적 피버 모드가 활성화됩니다.
  - 피버 모드 시 화면의 모든 파이프가 사라지고, 무지개 연출과 함께 코인들이 대량으로 뱀 형태의 물결을 이루며 쏟아집니다.
- **랜덤 아이템 박스 (Item Box)**:
  - 파이프가 생성될 때 20% 확률로 빈 공간(Gap)에 선물 상자가 생성됩니다.
  - 획득 시 5초 동안 자석(Magnet), 쉴드(Shield), 코인 2배(Double Coin), 혹은 즉시 피버 게이지 40% 충전(Fever Drink) 중 하나의 버프를 획득합니다.
